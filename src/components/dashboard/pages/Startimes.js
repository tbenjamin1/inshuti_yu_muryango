...
import {fetchAsynStartimesTransaction, getAllstartimesTransaction,} from '../../../redux/transactions/TransactionSlice';
function Startimes() {
    ...
    const startimesTransactionList = useSelector(getAllstartimesTransaction);

    useEffect(() => {
        dispatch(fetchAsynStartimesTransaction(selectedRange))
                ...
                <div className='bg-white my-4 rounded-lg border container ' >
                    ...
                    <div className='px-3 py-3'>
                        <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                            ...
                            {startimesTransactionList.length >0 ? (
                                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400 border">
                                    ...
                                    <tbody>
                                        {startimesTransactionList.map((transaction,index) => (
                                            <tr className="bg-white border-b hover:bg-gray-50 dark:hover:bg-gray-600" key={index} >
                                                ...
                                                <td className="px-6 py-4"  >
                                                    <span className="font-medium text-blue-600 dark:text-blue-500 hover:underline"  >{transaction.statusDesc?transaction.statusDesc:"N/A"}</span>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                        ...
                        {startimesTransactionList && (
                            <div className='flex justify-end my-3'>
                                <Pagination defaultCurrent={6} total={startimesTransactionList.length} className="border p-3 rounded-lg" />
                            </div>
                        )}
