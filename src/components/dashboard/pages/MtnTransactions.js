...
import { fetchAsyncMtnTransaction, getAllMTNTransaction } from '../../../redux/transactions/TransactionSlice';
function MtnTransactions() {
    ...
    const MTNtransactionList = useSelector(getAllMTNTransaction);
    const searchInTransactions = (searchQuery) => {
        let search = searchQuery.toLowerCase();
        if (search === "") {
            setFilteredTransactionList(MTNtransactionList);
        } else {
            let filteredList = MTNtransactionList.filter((transaction) => {
    ...
    useEffect(() => {
        dispatch(fetchAsyncMtnTransaction(selectedRange))
        ...
        <DashboardLayout>
            <div className='flex justify-center items-center' >
                <div className='bg-white my-4 rounded-lg border container ' >
                    <div className='border-b   px-3 py-3' > MTN transactions </div>
                    <div className='px-3 py-3'>
                                    ...
                                    <tbody>

                                        {
                                            filteredTransactionList.length ? (filteredTransactionList.map((transaction, index) => (
                                                ...
                                                MTNtransactionList.map((transaction, index) => (
                                                    <tr className="bg-white border-b hover:bg-gray-50 dark:hover:bg-gray-600" key={index} >
                                                        <td className="w-4 p-4">
                                                            <div className="flex items-center">
                                                                <input id="checkbox-table-search-1" type="checkbox" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                        ...
                        {MTNtransactionList && (
                            <div className='flex justify-end my-3'>
                                <Pagination defaultCurrent={6} total={MTNtransactionList.length} className="border p-3 rounded-lg" />
                            </div>
                        )}
...
export default MtnTransactions
